...
import type { Application, EntityRecord, Module, Role, User } from "$lib/data";
...
export const sectionMeta = {
  aplicaciones: {
    title: "Aplicaciones",
    singular: "aplicación",
    subtitle: "Administra las aplicaciones disponibles en tu organización.",
    icon: AppWindow,
  },
  modulos: {
    title: "Módulos",
    singular: "módulo",
    subtitle: "Organiza las funcionalidades de cada aplicación.",
    icon: Boxes,
  },
  roles: {
    title: "Roles",
    singular: "rol",
    subtitle: "Define accesos y relaciónalos con los módulos necesarios.",
    icon: ShieldCheck,
  },
  usuarios: {
    title: "Usuarios",
    singular: "usuario",
    subtitle: "Gestiona usuarios y asigna sus roles de acceso.",
    icon: Users,
  },
} as const;
...
export function validateForm(section: DataSection, form: FormState) {
  ...
  if (section === "aplicaciones" && form.link) {
    try {
      ...
      throw new Error("El enlace de la aplicación no es válido.");
    }
  }
  if (section === "modulos") {
  ...
  switch (section) {
    case "aplicaciones":
      return applications.map((record) => ({
        ...
        detail: record.link ?? "Sin enlace",
        relation: record.link ?? "—",
        active: record.active,
        raw: record,
      }));
    case "modulos":
      return modules.map((record) => ({
        id: record.id,
        name: record.name ?? "Sin nombre",
        detail:
    ...
    case "roles":
      return roles.map((record) => ({
        id: record.id,
        name: record.name ?? "Sin nombre",
        detail: record.alias ?? "Sin alias",
        relation: "Gestionar módulos",
        active: record.active,
        raw: record,
      }));
    case "usuarios":
      return users.map((record) => ({
        id: record.id,
        name: record.name ?? "Sin nombre",
        detail: record.email ?? "Sin correo",
        relation: "Gestionar roles",
        active: record.active,
        raw: record,
      }));
    case "inicio":
